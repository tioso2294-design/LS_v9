...
function App() {
  ...
  const getTopRestaurantsByRevenue = () => {
    return restaurants
      .filter(restaurant => restaurant.total_revenue && restaurant.total_revenue > 0)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-6">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div key={plan} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-gray-900 capitalize">
                          {plan === 'semiannual' ? 'Semi-annual' : plan} Plan
                        </span>
                      </div>
                      <div className="text-right">
              ...
              {/* Platform Growth Data Table */}
              <div className="bg-white rounded-2xl p-6 border border-gray-200">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Platform Growth (Last 6 Months)</h3>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="text-left py-2 px-3 text-sm font-medium text-gray-900">Month</th>
                        <th className="text-left py-2 px-3 text-sm font-medium text-gray-900">Restaurants</th>
                        <th className="text-left py-2 px-3 text-sm font-medium text-gray-900">Customers</th>
                        <th className="text-left py-2 px-3 text-sm font-medium text-gray-900">Revenue</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                      {growthData.map((data, index) => (
                        <tr key={index} className="hover:bg-gray-50">
                          <td className="py-2 px-3 font-medium text-gray-900">{data.month}</td>
                          <td className="py-2 px-3 text-gray-600">{data.restaurants}</td>
                          <td className="py-2 px-3 text-gray-600">{data.customers.toLocaleString()}</td>
                          <td className="py-2 px-3 text-gray-600">{formatCurrency(data.revenue)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-200">
                ...
                <div className="space-y-3">
                  {getTopRestaurantsByRevenue().map((restaurant, index) => (
                    <div key={restaurant.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{restaurant.total_customers || 0} customers</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-gray-900">{formatCurrency(restaurant.total_revenue || 0)}</p>
                        <p className="text-xs text-gray-500">revenue</p>
                      </div>
                    </div>
                  ))}
                  {getTopRestaurantsByRevenue().length === 0 && (
        ...
        {/* Analytics Tab with Professional Data Tables */}
        {activeTab === 'analytics' && (
          <div className="space-y-6">
            <div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-200">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Growth Analysis (Last 6 Months)</h3>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="text-left py-3 px-3 text-sm font-medium text-gray-900">Period</th>
                        <th className="text-left py-3 px-3 text-sm font-medium text-gray-900">Restaurants</th>
                        <th className="text-left py-3 px-3 text-sm font-medium text-gray-900">Customers</th>
                        <th className="text-left py-3 px-3 text-sm font-medium text-gray-900">Revenue</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                      {growthData.map((data, index) => {
                        const prevData = index > 0 ? growthData[index - 1] : null;
                        const restaurantGrowth = prevData ? data.restaurants - prevData.restaurants : 0;
                        const customerGrowth = prevData ? data.customers - prevData.customers : 0;
                        const revenueGrowth = prevData ? data.revenue - prevData.revenue : 0;
                        
                        return (
                          <tr key={index} className="hover:bg-gray-50">
                            <td className="py-3 px-3 font-medium text-gray-900">{data.month}</td>
                            <td className="py-3 px-3">
                              <div className="text-gray-900 font-medium">{data.restaurants}</div>
                              {restaurantGrowth !== 0 && (
                                <div className={`text-xs ${restaurantGrowth > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                  {restaurantGrowth > 0 ? '+' : ''}{restaurantGrowth}
                                </div>
                              )}
                            </td>
                            <td className="py-3 px-3">
                              <div className="text-gray-900 font-medium">{data.customers.toLocaleString()}</div>
                              {customerGrowth !== 0 && (
                                <div className={`text-xs ${customerGrowth > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                  {customerGrowth > 0 ? '+' : ''}{customerGrowth.toLocaleString()}
                                </div>
                              )}
                            </td>
                            <td className="py-3 px-3">
                              <div className="text-gray-900 font-medium">{formatCurrency(data.revenue)}</div>
                              {revenueGrowth !== 0 && (
                                <div className={`text-xs ${revenueGrowth > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                  {revenueGrowth > 0 ? '+' : ''}{formatCurrency(revenueGrowth)}
                                </div>
                              )}
                            </td>
                          </tr>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-500">
                            <span>{restaurant.total_customers || 0} customers</span>
                            <span>{restaurant.total_rewards || 0} rewards</span>
                            <span>{restaurant.total_points_issued?.toLocaleString() || 0} points</span>
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-lg text-gray-900">{formatCurrency(restaurant.total_revenue || 0)}</p>
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-200">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Subscription Plan Analysis</h3>
                <div className="space-y-3">
                  {Object.entries(subscriptionStats.planDistribution).map(([plan, count]) => {
                    const percentage = subscriptionStats.totalUsers > 0 ? (count / subscriptionStats.totalUsers) * 100 : 0;
                    const planRevenue = plan === 'monthly' ? count * 2.99 : 
                                       plan === 'semiannual' ? count * 9.99 / 6 :
                                       plan === 'annual' ? count * 19.99 / 12 : 0;
                      ...
                      <div key={plan} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">{percentage.toFixed(1)}% of users</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-gray-900">{count} users</p>
                          <p className="text-sm text-gray-500">{formatCurrency(planRevenue)}/mo</p>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-200">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Platform Health Dashboard</h3>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-green-50 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-green-700">All connections operational</p>
                      </div>
                    </div>
                    <span className="text-sm font-bold text-green-900">99.9% uptime</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-blue-50 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-blue-700">All endpoints responding</p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-purple-50 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-purple-900">Platform Load</p>
                        <p className="text-sm text-purple-700">Normal traffic levels</p>
                      </div>
                    </div>
                    <span className="text-sm font-bold text-purple-900">Optimal</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-yellow-50 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-yellow-900">Support Queue</p>
                        <p className="text-sm text-yellow-700">Active ticket monitoring</p>
                      </div>
                    </div>
                    <span className="text-sm font-bold text-yellow-900">{systemStats?.active_tickets || 0} pending</span>
                  </div>
                </div>
              </div>
            </div>
